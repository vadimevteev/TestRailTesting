...
namespace TestRailAutomationTest.Page.TestCase.CreateTestCasePage.Section
{
    public class StepsSection
    {
        ...
        private const string AddTestCaseButtonPath =
            "//div[@id=\"custom_steps_separated_container\"]//a[@class=\"addStep\"]";
        ...
        private static string BuildStepTextAreaXpath(string label) =>
            $"//td[@class=\"step-content\"]//div[@placeholder=\"{label}\"]";
        ...
        private Textarea DescriptionArea => new(_driver, BuildStepTextAreaXpath(TestCaseProperties.StepDescriptionName),
            TestCaseProperties.StepDescriptionName);

        private Textarea ExpectedResultArea => new(_driver,
            BuildStepTextAreaXpath(TestCaseProperties.StepExpectedResultName),
            TestCaseProperties.StepExpectedResultName);


        public void FillSteps(BaseTestCase testCase)
        {
            PreconditionsArea(TestCaseProperties.Preconditions)
                .SetValueAfterClick(((StepsTestCase) testCase).Preconditions);
            AddStepButton.Click();
